...
import io.github.dimpon.ClassE;
...
public class TestprivateTest {
  ...
  void callPrivateStaticMethod() {
    TriplicateString triplicateString = lookupPrivatesIn(ObjectWithPrivateMethod.class).usingInterface(
        TriplicateString.class);
  ...
  void testMethodNotFound() {
    ...
    Assertions.assertEquals(
        "Method (or field matches to method) public abstract java.lang.String io.github.dimpon.tests.TestprivateTest$ToLowerCase.nameInLowerCase() is not found in original class/object",
        ex.getMessage());
  }
  ...
  void testMethodWithException() {
    WithExceptions withExceptions = lookupPrivatesIn(new ClassE()).usingInterface(WithExceptions.class);

    IllegalArgumentException e1 = Assertions.assertThrows(IllegalArgumentException.class,
        withExceptions::throwRuntimeException);

    Assertions.assertEquals("argument is illegal", e1.getMessage());

    ClassE.ClassEException e2 = Assertions.assertThrows(ClassE.ClassEException.class,
        withExceptions::throwException);

    Assertions.assertEquals("my custom exception", e2.getMessage());
  }
  ...
  interface WithExceptions {
    void throwRuntimeException();

    void throwException() throws io.github.dimpon.ClassE.ClassEException;
  }
